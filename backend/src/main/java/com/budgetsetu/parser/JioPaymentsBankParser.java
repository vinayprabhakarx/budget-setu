...
public class JioPaymentsBankParser extends BaseBankParser {
    ...
    public List<Map<String, String>> parseText(String text, String fileName) {
        ...
        for (List<String> block : blocks) {
            ...
            if (m3.find()) {
                ...
                if (narration.contains("/CR/"))
                    type = "CREDIT";
                else if (narration.contains("/DR/"))
                    type = "DEBIT";
                else if (narration.toUpperCase().contains(" CR.") || narration.toUpperCase().contains("CREDIT"))
                    type = "CREDIT";
                else
                    type = "DEBIT";
            } else {
            ...
            // Parse Jio Payments Bank Narration:
            // UPI/{direction}/{transaction_id}/{counterparty_name}/{bank_code}/{upi_id}/{remarks}
            if (narration.contains("UPI/")) {
                ...
                if (parts.length >= 4) {
                    if (type.equals("DEBIT"))
                        txn.put("payee", parts[3]);
                    else
                        txn.put("payer", parts[3]);
                }
                if (parts.length >= 6)
                    txn.put("upi_id", parts[5] + "@" + parts[4].toLowerCase());
                if (parts.length >= 7) {
                    String rem = parts[6].trim();
                    if (rem.equals("NA") || rem.equals("NO REMARKS"))
                        rem = null;
                    txn.put("remarks", rem);
                }
    ...
    public boolean canHandle(String text, String fileName) {
        if (text == null)
            return false;
        return text.contains("Jio Payments Bank") || text.contains("JIO PAYMENTS BANK")
                || text.contains("Jio Payment Bank");
    }
