    ...
    public List<Map<String, String>> parseText(String text, String fileName) {
        ...
        for (List<String> block : blocks) {
            ...
            if (m3.find()) {
                ...
                narration = cleanLine
                        .substring(dateMatcher.group(0).length(), cleanLine.length() - m3.group(0).length()).trim();
            } else if (m2.find()) {
                amount = m2.group(1).replace(",", "");
                balance = m2.group(2).replace(",", "");
                narration = cleanLine
                        .substring(dateMatcher.group(0).length(), cleanLine.length() - m2.group(0).length()).trim();
            ...
            // UPI/{payee_name}/{upi_id}/{remarks}/{bank_name}/{transaction_id}/...
            if (narration.contains("UPI/")) {
                ...
                if (parts.length >= 6)
                    txn.put("transaction_id", parts[5]);
            } else if (narration.contains("VIN/") || narration.contains("INF/")) {
                txn.put("mode", "NETBANKING");
                String netStr = narration.contains("VIN/") ? narration.substring(narration.indexOf("VIN/"))
                        : narration.substring(narration.indexOf("INF/"));
                ...
                if (parts.length >= 4)
                    txn.put("transaction_id", parts[3]);
