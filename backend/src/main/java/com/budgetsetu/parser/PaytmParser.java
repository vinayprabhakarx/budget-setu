...
public class PaytmParser extends BaseBankParser {
    ...
    private static final Pattern DATE_LEAD = Pattern.compile("^\\s*(\\d{1,2}[-\\s]+[A-Za-z]{3}(?:[-\\s,]*\\d{2,4})?|\\d{1,2}[-/.]\\d{1,2}[-/.]\\d{2,4})");
            ...
            if (amtMatcher.find()) {
                ...
                if ("CREDIT".equals(type)) {
                    if (fullText.contains("Received from")) {
                        ...
                        if (party == null) party = extractBetween(fullText, "Received from", "Rs.");
                    ...
                    if (fullText.contains("Paid to")) {
                        ...
                        if (party == null) party = extractBetween(fullText, "Paid to", "Rs.");
                    } else if (fullText.contains("Money sent to")) {
                        party = extractBetween(fullText, "Money sent to", "UPI ID:");
                        if (party == null) party = extractBetween(fullText, "Money sent to", "Rs.");
                    } else if (fullText.contains("Paid for")) {
                        party = extractBetween(fullText, "Paid for", "UPI ID:");
                        if (party == null) party = extractBetween(fullText, "Paid for", "Rs.");
                    }
                }
    ...
    public boolean canHandle(String text, String fileName) {
        ...
        return text.contains("Paytm Statement for") || text.contains("payments done by you on Paytm App") || (fileName != null && fileName.toLowerCase().contains("paytm"));
    }
