...
export const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ isOpen, onClose }) => {
  ...
  const fetchMeta = useCallback(async (showLoader = true) => {
    ...
    try {
      ...
      if (accRes.data.length > 0) setAccountId(prev => prev ? prev : accRes.data[0].id);
      if (catRes.data.length > 0) setCategoryId(prev => prev ? prev : catRes.data[0].id);
      
      hasData.current = true;
    } catch (err) {
      console.error('Failed to load accounts/categories', err);
      if (showLoader) showToast('error', 'Failed to retrieve accounts and categories list.');
    } finally {
  ...
  const validate = () => {
    ...
    if (!amount || Number(amount) <= 0) tempErrors.amount = 'Please enter a positive amount';
    ...
    <div className="modal-overlay">
      <div className="modal max-w-lg">
        <div className="modal-header">
          <h3 className="font-display text-text-primary text-heading-md">Add Transaction</h3>
          <button onClick={onClose} className="text-text-secondary hover:text-text-primary p-1 rounded-md hover:bg-bg-subtle cursor-pointer">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
            <div className="modal-body space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {errors.transactionDate && <p className="text-destructive text-body-sm mt-1">{errors.transactionDate}</p>}
                </div>
              </div>
              ...
              <div>
                ...
                {accounts.length === 0 && !loading ? (
                  ...
                  <Select
                    ...
                    options={accounts.map(acc => ({ value: acc.id, label: `${acc.name} (${acc.bankName} - ${acc.accountNumber})` }))}
                    error={!!errors.accountId}
                    disabled={loading}
                    placeholder={loading ? "Fetching accounts..." : "Select Account"}
                  />
                )}
                {errors.accountId && <p className="text-destructive text-body-sm mt-1">{errors.accountId}</p>}
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  disabled={loading}
                  placeholder={loading ? "Fetching categories..." : "Select Category"}
                />
                {errors.categoryId && <p className="text-destructive text-body-sm mt-1">{errors.categoryId}</p>}
              </div>
              ...
              <div>
                ...
                />
                {errors.payee && <p className="text-destructive text-body-sm mt-1">{errors.payee}</p>}
              </div>
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                className="btn btn-primary flex items-center justify-center min-w-32"
              >
                {submitting ? (
                  <>
