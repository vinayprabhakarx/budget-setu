...
public class PnbParser extends BaseBankParser {
    ...
    public List<Map<String, String>> parseText(String text, String fileName) {
        ...
        for (List<String> block : blocks) {
            ...
            // Actually groupIntoBlocks doesn't modify the line characters, but
            // String.join(" ", block) adds spaces.
            ...
            // Wait, for Excel the date might look like '30-Apr-2026', let's adjust
            // DATE_LEAD
            // Let's just match the start of string

            // If the row doesn't match flexPattern, we check if it matches without
            // instrument ID.
            ...
            Map<String, String> txn = ParserUtil.emptyTransaction();
            // Need the actual date string from fullText, but wait, altPattern doesn't
            // capture date.
    ...
    private void processNarration(Map<String, String> txn) {
        ...
        if (narration.contains("UPI/")) {
            ...
            if (parts.length >= 4) {
                if ("DEBIT".equals(typeIndicator))
                    txn.put("payee", parts[3]);
                else
                    txn.put("payer", parts[3]);
            }
            if (parts.length >= 6)
                txn.put("upi_id", parts[5]);
