...
export const EmailVerificationCode: React.FC = () => {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      navigate(`/verify-email?email=${encodeURIComponent(emailForResend)}`);
    } catch (err: unknown) {
      const error = err as { response?: { data?: { message?: string } } };
      const msg = error.response?.data?.message || 'Failed to resend verification link.';
      showToast('error', msg);
    } finally {
      setResending(false);
    }
      ...
      <div className="grow flex items-center justify-center px-4 py-12 md:py-20">
        <div className="w-full max-w-110 text-center space-y-7">
          ...
          <MagicLinkState
            ...
            codeDesc="Enter this code on the verification page to confirm your email."
            ...
            onGoToFallback={() => navigate('/verify-email')}
            goToFallbackLabel="Go to Verification Page"
          />
